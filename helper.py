...
def parse_quintuple(line: str, granularity: str):
    ...
    def parse_time(time: str, fallback: int, granularity: str):
        # Year level granularity parser.
        if granularity == "year":
            # NOTE: This implementation ignores years taht consist of fewer than 4 digits.
            #       This is the same behaviour as the original HyTE.
            time = time.split("-")[0]
            if time == "####":
                return fallback
            elif time.find("#") != -1 or len(time) != 4:
                return -1
            return int(time)

        # Day level granularigty parser
