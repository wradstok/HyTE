...
train = pd.read_table(
    "train.txt", header=None, names=["sub", "pred", "obj", "time_begin"]
)
test = pd.read_table(
    "test.txt", header=None, names=["sub", "pred", "obj", "time_begin"]
)
valid = pd.read_table(
    "valid.txt", header=None, names=["sub", "pred", "obj", "time_begin"]
)
...
for i, split in enumerate([train, test, valid, triple2id]):
    ...
    split.to_csv(
        ...
        sep="\t",
        header=False,
        columns=["sub", "pred", "obj", "time_begin", "time_end"],
        index=False,
    )
