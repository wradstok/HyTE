...
def parse_args():
	...
	parser.add_argument('-eval_metric', dest="eval_metric", required=True, choices=eval_metrics.keys(), help="Which temporal evaluation metric to apply.")
...
for k in range(args.freq,30000,args.freq):
	...
	print(f'Epoch {k} : MR {np.mean(np.array(ranks))}')