...
def test_link_prediction(self):
    ...
    def f_head():
        ...
        pos_t_e = tf.reshape(
            tf.tile(e2, [self.max_ent]), (self.max_ent, self.p.inp_dim)
        )
    ...
    def f_tail():
        ...
        pos_h_e = tf.reshape(
            tf.tile(e1, [self.max_ent]), (self.max_ent, self.p.inp_dim)
        )
...
def calculated_score_for_positive_elements(
    self, sess, quintuple, epoch, f_valid, eval_mode="valid"
):
    ...
    pos_head = sess.run(
        self.pos,
        feed_dict={
            self.pos_head: np.array([quintuple[0]]).reshape(-1, 1),
...
def temp_test_against(self, sess, data, mode: str, epoch: int):
    ...
    for i, quintuple in enumerate(data):
        start_lbl, end_lbl = Helper.get_span_ids(
            quintuple[3], quintuple[4], self.year2id
        )
...
def test_link_against(self, sess, data, mode: str, epoch: int):
    ...
    if epoch == self.p.test_freq:
        ...
        for i, t in enumerate(data):
            score = calculated_score_for_positive_elements(
                t, sess, epoch, f_valid, mode
            )
            if score:
