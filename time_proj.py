...
class HyTE(Model):
    ...
    def load_data(self):
        ...
        with open(self.p.triple2id, "r") as filein:
            for line in filein:
                h, r, t, _, _ = Helper.parse_quintuple(line, self.p.granularity)
    ...
    def add_model(self):
        with tf.name_scope("embedding"):
            self.ent_embeddings = tf.get_variable(
                name="ent_embedding",
                shape=[self.max_ent, self.p.inp_dim],
                initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                regularizer=self.regularizer,
            )
            self.rel_embeddings = tf.get_variable(
                name="rel_embedding",
                shape=[self.max_rel, self.p.inp_dim],
                initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                regularizer=self.regularizer,
            )
            self.time_embeddings = tf.get_variable(
                name="time_embedding",
                shape=[self.max_time, self.p.inp_dim],
                initializer=tf.contrib.layers.xavier_initializer(uniform=False),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-data_type", dest="data_type", default="yago", choices=["yago", "wikidata", "icews"], help="dataset to choose"
    )
    ...
    parser.add_argument(
        "-onlytransE", dest="onlytransE", action="store_true", help="Evaluate model on only transE loss"
    )
    parser.add_argument(
        "-restore", dest="restore", action="store_true", help="Restore from the previous best saved model"
    )
    parser.add_argument(
        "-res_epoch", dest="restore_epoch", default=200, type=int, help="Restore from the previous best saved model"
    )
